...
// GET grade-level distribution summary (cached, 60s TTL)
// Auth runs BEFORE cache read (contract §3). X-Cache header advisory (§2 headers).
...
router.get('/grade-summary', requireAuth, accountingAndWarehouse, async (req, res) => {
  try {
    ...
    const cacheKey = `report:grade-summary:${academicYear}:${term}`;
    let cacheHit = true;
...
// GET profitability per inventory item (cached, 60s TTL)
// Auth runs BEFORE cache read (contract §3). X-Cache header advisory (§2 headers).
...
router.get('/profitability', requireAuth, accountingAndWarehouse, async (req, res) => {
  try {
    ...
    const cacheKey = `report:profitability:${academicYear}:${term ?? 'all'}`;
    let cacheHit = true;
