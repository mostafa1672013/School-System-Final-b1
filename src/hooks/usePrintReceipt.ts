...
<body>
  <div class="receipt-card">
    ...
    <div class="footer">
      <p>تم إصدار هذا الإيصال إلكترونياً من خلال نظام إدارة مدرستي</p>
      <p>تاريخ ووقت الطباعة: ${new Date().toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
    </div>
  </div>

  <div class="no-print">
    <button class="btn btn-print" onclick="window.print()">🖨️ طباعة الإيصال</button>
  ...
  try {
    ...
    const printWindow = window.open(url, '_blank', 'width=900,height=750');
    if (!printWindow) {
