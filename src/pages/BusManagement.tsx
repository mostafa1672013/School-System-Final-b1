            ...
            <div className="rounded-lg border bg-card">
              {selectedRouteData ? (
                <div>
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-center mb-3">
                      <p className="text-sm font-medium">المشتركين ({routeSubscribers.length})</p>
                    </div>
                    <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                      {routeSubscribers.map((sub) => (
                        <div key={sub.id} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/40 text-sm border hover:border-slate-300 transition-all">
                          <div className="space-y-1 text-right">
                            <span className="font-medium block">{sub.subscriberName}</span>
                            <span className="text-xs text-muted-foreground font-mono">{SUBSCRIBER_LABEL[sub.subscriberType]}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Button
                              variant="ghost"
                              size="icon"
                              className="size-8 rounded-lg text-amber-600 hover:text-amber-700 hover:bg-amber-50"
                              onClick={() => {
                                setSelectedSubForChange(sub);
                                setChangeForm((f) => ({ ...f, newRouteId: '' }));
                                setChangeDialogOpen(true);
                              }}
                              title="تغيير أو إلغاء الاشتراك"
                            >
                              <ArrowLeftRight className="size-4" />
                            </Button>
                            <button
                              onClick={() => {
                                if (confirm(`هل أنت متأكد من إلغاء اشتراك ${sub.subscriberName} نهائياً؟`)) {
                      ...
                      {routeSubscribers.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">لا يوجد مشتركين في هذا الخط</p>}
                    </div>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
                  ...
                  <p className="text-sm">اختر خطاً لعرض التفاصيل والمشتركين</p>
                </div>
              )}
            </div>
          </div>
        ...
        <TabsContent value="fleet" className="mt-6 space-y-4">
          <div className="flex justify-end">
            <Dialog open={busDialogOpen} onOpenChange={setBusDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader><DialogTitle className="font-[Noto_Kufi_Arabic]">إضافة باص جديد للأسطول</DialogTitle></DialogHeader>
                <form onSubmit={async (e) => {
                  e.preventDefault();
                  await addBus({
                    ...busForm,
                    capacity: Number(busForm.capacity),
                    year: Number(busForm.year),
                  });
