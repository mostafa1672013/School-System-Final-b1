# Demo Users & Seed Data Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `student_affairs` role to the type system and create a comprehensive demo seed script that populates 6 user accounts and 110 students with realistic daily-work scenarios for every role.

**Architecture:** Two independent changes — (1) a one-line type change for the new role, (2) a standalone `server/prisma/seed_demo.ts` script that uses Prisma upserts to safely seed all demo data. The script is divided into helper sections: users → inventory items → students by batch → financial scenarios (payments, installments, inventory transactions).

**Tech Stack:** TypeScript, Prisma (SQLite), bcrypt, ts-node.
...
| File | Change |
|------|--------|
| `src/types/index.ts` | Add `'student_affairs'` to `UserRole` union |
| `server/prisma/seed_demo.ts` | New: full demo seed script |

---

### Task 1: Add `student_affairs` to UserRole

**Files:**
- Modify: `src/types/index.ts` (line 1)

- [ ] **Step 1: Update UserRole in src/types/index.ts**
...
git commit -m "feat: add student_affairs role to UserRole type"
```

---

### Task 2: Create seed_demo.ts — users + inventory items

**Files:**
- Create: `server/prisma/seed_demo.ts`

- [ ] **Step 1: Create the file with imports, users, and inventory items**
...
✅ تم إضافة 6 مستخدمين.
📦 جاري إضافة أصناف المخزن...
✅ تم إضافة 5 أصناف مخزن.
```
...
```bash
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed demo users and inventory items"
```

---

### Task 3: Seed students — admitted students (batches 1–5)

**Files:**
- Modify: `server/prisma/seed_demo.ts`

This task adds 92 admitted/active students across all stages.

- [ ] **Step 1: Add seedAdmittedStudents() function**

Add this function before `main()` in `server/prisma/seed_demo.ts`:
...
async function seedAdmittedStudents() {
  ...
  const students = [
    ...
    // --- Preparatory Grade 1 (6 students) ---
    ...
    // --- Preparatory Grade 2 (6 students) ---
    ...
    // --- Preparatory Grade 3 (6 students) ---
...
Expected: 92 students added without errors.
...
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed 92 admitted students across all stages and grades"
```

---

### Task 4: Seed students — applied/under_testing (batch 6) + promoted with arrears (batch 7)

**Files:**
- Modify: `server/prisma/seed_demo.ts`

- [ ] **Step 1: Add seedPendingStudents() and seedPromotedStudents()**
...
async function seedPendingStudents() {
  ...
  const students = [
    { nationalId: '31005010000201', name: 'إسلام محمود السيد', stage: 'kg', grade: 'KG1', track: 'local', guardianName: 'محمود السيد عبدالله', guardianPhone: '01055555201', status: 'applied', hasSiblings: false, testResult: null },
    { nationalId: '31005010000202', name: 'مروة سعيد الديب', stage: 'kg', grade: 'KG2', track: 'local', guardianName: 'سعيد الديب طاهر', guardianPhone: '01055555202', status: 'applied', hasSiblings: false, testResult: null },
    { nationalId: '31005010000203', name: 'حازم عصام الأمير', stage: 'primary', grade: 'الصف الأول الابتدائي', track: 'local', guardianName: 'عصام الأمير رشاد', guardianPhone: '01055555203', status: 'applied', hasSiblings: false, testResult: null },
    { nationalId: '31005010000204', name: 'إيمان تامر الكردي', stage: 'primary', grade: 'الصف الثاني الابتدائي', track: 'local', guardianName: 'تامر الكردي وليد', guardianPhone: '01055555204', status: 'applied', hasSiblings: false, testResult: null },
    { nationalId: '31005010000205', name: 'سعد يوسف البدوي', stage: 'kg', grade: 'KG1', track: 'international', guardianName: 'يوسف البدوي سامر', guardianPhone: '01055555205', status: 'applied', hasSiblings: false, testResult: null },
    { nationalId: '31005010000211', name: 'نادية حسن الجمال', stage: 'kg', grade: 'KG1', track: 'local', guardianName: 'حسن الجمال علي', guardianPhone: '01055555211', status: 'under_testing', hasSiblings: false, testResult: null },
    { nationalId: '31005010000212', name: 'طارق فتحي النخيلي', stage: 'primary', grade: 'الصف الأول الابتدائي', track: 'local', guardianName: 'فتحي النخيلي أحمد', guardianPhone: '01055555212', status: 'under_testing', hasSiblings: false, testResult: null },
    { nationalId: '31005010000213', name: 'أميرة خليل السبكي', stage: 'primary', grade: 'الصف الثالث الابتدائي', track: 'local', guardianName: 'خليل السبكي عمر', guardianPhone: '01055555213', status: 'under_testing', hasSiblings: false, testResult: null },
    { nationalId: '31005010000214', name: 'ياسر محمد الفندي', stage: 'kg', grade: 'KG2', track: 'local', guardianName: 'محمد الفندي طلعت', guardianPhone: '01055555214', status: 'under_testing', hasSiblings: false, testResult: null },
    { nationalId: '31005010000215', name: 'ربا سمير مرقس', stage: 'primary', grade: 'الصف الثاني الابتدائي', track: 'local', guardianName: 'سمير مرقس ميلاد', guardianPhone: '01055555215', status: 'under_testing', hasSiblings: false, testResult: null },
  ];
...
async function seedPromotedStudents() {
  ...
  for (const s of promoted) {
    ...
    // Create old-year yearlyFinance record
    await prisma.studentYearlyFinance.upsert({
      ...
      create: {
        ...
        stage: oldStage,
        grade: oldGrade,
        tuitionFees: oldTuitionFees,
        booksFees: oldBooksFees,
        uniformFees: oldUniformFees,
        busFees: 0,
        otherFees: 0,
        arrearsFees: 0,
...
Expected: 10 pending students + 8 promoted students with yearlyFinance records added.
...
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed pending/under_testing students and promoted students with arrears"
```

---

### Task 5: Seed financial data — payments + pending requests

**Files:**
- Modify: `server/prisma/seed_demo.ts`

- [ ] **Step 1: Add seedPayments() function**
...
async function seedPayments() {
  ...
  const year = '2024-2025';
  // National IDs of active students who will get payment records
  const paidStudentIds = [
    '31001010000001', '31001010000003', '31001010000004', '31001010000006',
    '31001010000008', '31001010000013', '31001010000014', '31001010000015',
    '31002010000021', '31002010000022', '31002010000024', '31002010000026',
    '31002010000031', '31002010000032', '31002010000034', '31002010000036',
    '31002010000041', '31002010000042', '31002010000044', '31002010000046',
    '31002010000051', '31002010000052', '31002010000054', '31002010000056',
    '31003010000081', '31003010000082', '31003010000084', '31003010000086',
    '31003010000091', '31003010000092', '31003010000094', '31003010000096',
    '31004010000111', '31004010000112', '31004010000114', '31004010000116',
    '31004010000121', '31004010000122', '31004010000124', '31004010000126',
    '31004010000131', '31004010000132', '31004010000134', '31004010000136',
  ];

  // National IDs of students with pending payment requests
  const pendingStudentIds = [
    '31001010000007', '31001010000009', '31001010000011',
    '31002010000023', '31002010000025', '31002010000033', '31002010000035',
    '31003010000083', '31003010000085', '31003010000093', '31003010000095',
    '31004010000113', '31004010000115', '31004010000123', '31004010000125',
  ];

  let receiptCounter = 1000;

  // Seed approved payments for active students
  for (const nationalId of paidStudentIds) {
    const student = await prisma.student.findUnique({ where: { nationalId } });
    if (!student) continue;

    const tuitionPaid = Math.floor(student.tuitionFees * 0.6);
    await prisma.payment.upsert({
      where: { receiptNumber: `RCP-${receiptCounter}` },
      update: {},
      create: {
        studentId: student.id,
        ...
        amount: tuitionPaid,
        type: 'tuition',
        method: receiptCounter % 3 === 0 ? 'cash' : receiptCounter % 3 === 1 ? 'bank_transfer' : 'check',
        date: '2024-10-01',
        receiptNumber: `RCP-${receiptCounter}`,
        collectedBy: 'محمود إبراهيم نصر',
        academicYear: year,
        notes: 'دفعة أولى',
      },
    });
    ...
    await prisma.student.update({
      where: { id: student.id },
      data: { paidAmount: tuitionPaid + student.booksFees },
    });
  }

  // Seed pending payment requests
  for (const nationalId of pendingStudentIds) {
    const student = await prisma.student.findUnique({ where: { nationalId } });
    if (!student) continue;
    ...
    await prisma.student.update({
      where: { id: student.id },
      data: {
        pendingPaymentAmount: Math.floor(student.tuitionFees * 0.4),
        pendingPaymentType: 'tuition',
        pendingPaymentMethod: 'bank_transfer',
        paymentRequestStatus: 'pending',
        pendingPaymentNotes: 'طلب سداد دفعة ثانية من الرسوم الدراسية',
      },
    });
  }

  console.log(`✅ تم إضافة مدفوعات لـ ${paidStudentIds.length} طالب و${pendingStudentIds.length} طلب دفع معلق.`);
}
```
...
Expected: payments added, 15 students with `paymentRequestStatus: 'pending'`.
...
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed approved payments and pending payment requests"
```

---

### Task 6: Seed installment plans

**Files:**
- Modify: `server/prisma/seed_demo.ts`

- [ ] **Step 1: Add seedInstallmentPlans() function**
...
async function seedInstallmentPlans() {
  ...
  // Students who will get installment plans (mix of active + overdue)
  const planStudents = [
  ...
  for (const { nationalId, overdue } of planStudents) {
    ...
    const installmentAmount = Math.floor(remaining / 4);

    const installments = overdue
      ? [
          { amount: installmentAmount, dueDate: '2024-10-15', paidAmount: 0, status: 'pending' as const },
          { amount: installmentAmount, dueDate: '2024-11-15', paidAmount: 0, status: 'pending' as const },
          { amount: installmentAmount, dueDate: '2025-01-15', paidAmount: 0, status: 'pending' as const },
          { amount: remaining - installmentAmount * 3, dueDate: '2025-03-15', paidAmount: 0, status: 'pending' as const },
        ]
      : [
          { amount: installmentAmount, dueDate: '2024-10-15', paidAmount: installmentAmount, paidDate: '2024-10-10', status: 'paid' as const },
          { amount: installmentAmount, dueDate: '2024-12-15', paidAmount: 0, status: 'pending' as const },
          { amount: installmentAmount, dueDate: '2025-02-15', paidAmount: 0, status: 'pending' as const },
          { amount: remaining - installmentAmount * 3, dueDate: '2025-04-15', paidAmount: 0, status: 'pending' as const },
        ];
  ...
  console.log(`✅ تم إضافة خطط أقساط لـ ${planStudents.length} طالب.`);
}
```
...
Expected: 25 installment plans created (15 active, 10 overdue).
...
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed installment plans (active and overdue scenarios)"
```

---

### Task 7: Seed inventory transactions

**Files:**
- Modify: `server/prisma/seed_demo.ts`

- [ ] **Step 1: Add seedInventoryTransactions() function**
...
async function seedInventoryTransactions() {
  ...
  // Students who bought books or uniforms
  const bookBuyers = [
    '31002010000021', '31002010000022', '31002010000024', '31002010000026',
    '31002010000031', '31002010000032', '31002010000034',
    '31003010000081', '31003010000082', '31003010000084',
    '31003010000091', '31003010000092', '31003010000094',
    '31003010000101', '31003010000102', '31003010000104',
  ];

  const uniformBuyers = [
    '31001010000001', '31001010000003', '31001010000006', '31001010000008',
    '31002010000021', '31002010000022', '31002010000031', '31002010000041',
    '31002010000051', '31002010000061', '31002010000071',
    '31003010000081', '31003010000082',
    '31004010000111', '31004010000112',
    '31006010000301', '31006010000303',
  ];
  ...
  for (const nationalId of bookBuyers) {
    ...
    await prisma.inventoryTransaction.create({
      data: {
        ...
        studentId: student.id,
        studentName: student.name,
        performedBy: 'سامي عبدالله حسين',
        date: '2024-09-20',
        notes: 'بيع كتاب للطالب',
      },
    });
    txIndex++;
  }
  ...
  for (const nationalId of uniformBuyers) {
    ...
    await prisma.inventoryTransaction.create({
      data: {
        ...
        performedBy: 'سامي عبدالله حسين',
        date: '2024-09-10',
        notes: 'بيع زي صيفي للطالب',
      },
    });
    txIndex++;
  }

  // Incoming stock (purchase transactions)
  await prisma.inventoryTransaction.create({
    data: {
      itemId: 'inv-item-001',
      type: 'in',
      ...
      totalAmount: 30000,
      supplierName: 'دار النشر التعليمية',
      performedBy: 'سامي عبدالله حسين',
      date: '2024-09-01',
      notes: 'شراء مخزون بداية العام',
    },
  });
  ...
  await prisma.inventoryTransaction.create({
    data: {
      ...
      unitPriceSnapshot: 350,
      totalAmount: 100000,
      supplierName: 'مصنع الزي المدرسي',
      performedBy: 'سامي عبدالله حسين',
      date: '2024-08-25',
      notes: 'شراء مخزون الزي الصيفي',
    },
  });

  console.log(`✅ تم إضافة ${txIndex + 2} حركة مخزن.`);
}
```
...
Expected: 40+ inventory transactions created.
...
cd "/Users/me/Downloads/Project/untitled folder"
git add server/prisma/seed_demo.ts
git commit -m "feat: seed inventory transactions (sales and purchases)"
```
...
| Requirement | Task |
|------------|------|
| Add `student_affairs` to UserRole | Task 1 |
| 6 demo user accounts (one per role) | Task 2 |
| 5 inventory items | Task 2 |
| 92 admitted/active students across all stages | Task 3 |
| 10 students in applied/under_testing | Task 4 |
| 8 promoted students with yearlyFinance 2023-2024 + arrears | Task 4 |
| 44 approved payment records | Task 5 |
| 15 pending payment requests | Task 5 |
| 25 installment plans (15 active, 10 overdue) | Task 6 |
| 40+ inventory transactions (sales + purchases) | Task 7 |
| Discounts on ~20 students | Tasks 3+4 (inline in student data) |
| Sibling discounts | Tasks 3+4 (inline) |
| International track students | Task 3 (8 students) |
| Bus fee students | Tasks 3+4 (inline) |

All requirements covered. ✅

**Placeholder scan:** No TBD, no vague steps. All code blocks complete. ✅

**Type consistency:** All Prisma model fields match schema — `installmentPlan` uses `studentId` unique, `installments` nested create, `StudentYearlyFinance` uses `studentId_academicYear` composite key. ✅
